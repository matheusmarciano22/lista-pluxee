...
with col1:
    st.markdown("### ⚙️ Origem dos Dados da Empresa")
    ...
    if origem_dados == "Lovable (CRM)":
        ...
        st.info("Suba uma planilha padrão da Pluxee preenchida. O robô vai copiar o endereço e o responsável dela.")
        
        st.session_state.razao_social = st.text_input("Nome da Empresa (Para o arquivo final)", "Cliente_Legado")
        arq_antigo = st.file_uploader("Suba a planilha antiga (.xlsx)", type=["xlsx"])
        ...
        if arq_antigo:
            try:
                ...
                # A Pluxee sempre guarda os dados da empresa a partir da linha 8
                l_dados = 8 
                ...
                dados_e = {
                    ...
                    'Responsável': str(ws_antigo.cell(row=l_dados, column=25).value or "").replace('None', ''), # <-- Puxando da Coluna Y (25)
    ...
    st.session_state.config_rh = {
        ...
        'Responsável': st.text_input("Responsável", dados_e.get('Responsável', '')), # O Responsável vai aparecer aqui para você ver
...
with col2:
    st.markdown("### 📥 Ficheiro dos Novos Funcionários")
    arq = st.file_uploader("Suba Excel (.xlsx, .xls), CSV ou Word (.docx) dos novos colaboradores", type=["xlsx", "xls", "csv", "docx"])
                ...
                if st.button("🚀 Gerar Planilha Pluxee Oficial", use_container_width=True):
                    ...
                    for _, r in data_rows.iterrows():
                        ...
                        for p_code in ["6001 - Carteira Refeição", "6002 - Carteira Alimentação"]:
                            ...
                            # CÓDIGO ATUALIZADO: "023 - Pedido de 1ªVia de Cartão Sem Crédito" na Coluna K (11)
                            ws.cell(row=r_idx, column=11, value="023 - Pedido de 1ªVia de Cartão Sem Crédito")
                            ...
                            # Gravando o Responsável na Coluna Y (25)
